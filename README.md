...
- 독립 변수: <a href="https://kpa1959.or.kr/file/E104.pdf">관련 연구</a>(공공임대주택 입주의향 영향요인)에 사용된 변수들을 기반으로 구성
- 정책 기준에 부합하는 가구들을 필터링한 결과, 최종 데이터 수는 <b>46,796</b>

- 수치형 변수(예: 나이, 소득, 자산, 부채 등)의 이상치로 인한 영향을 최소화하고자 <b>RobustScaler</b> 사용
...
구체적으로, 첫 번째 방법은 두 클래스의 recall이 서로 비슷해지는(즉, $TPR \approx TNR$) 임곗값을 찾는 것이지만, Balanced Accuracy 기반 방법은 $t^* = \arg \max_{t} \dfrac{TPR(t) + TNR(t)}{2}$로, 두 클래스 recall의 평균이 최대가 되도록, 즉 $TPR$과 $TNR$의 합이 최대가 되게 하는 $t$를 찾는 것이다.

아래 그림은 train set에서 학습시킨 모델을 사용하여 valid set에서 두 방법으로 $t^*$를 찾은 결과로, 모두 $t^* \approx 0.3$ 근처에 위치하였다. 
<div align="center">
  <img src="img/fpr+tpr-1.png" alt="FPR+TPR 그래프" width="48%">
