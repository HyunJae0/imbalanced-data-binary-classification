...
## 개인 프로젝트
다음과 같은 식을 이용하여 최적의 임곗값(threshold)를 찾은 다음, 이 임곗값을 적용해 예측(분류)을 수행했습니다.
...
다만, 다운샘플링은 데이터를 축소하기 때문에 정보 손실이, 업샘플링은 주어진 데이터 셋에 대한 성능이 향상될 수는 있으나, unseen data에 대한 예측 성능이 좋지 않을 수 있습니다. (과적합)

그러므로 샘플링 기법을 사용하는 대신, 데이터에 기반하여 모델의 예측 확률값을 최종 클래스로 결정하는 기준점인 임곗값을 조정하는 접근법을 채택하였습니다. '입주 의향 있음'과 '없음' 두 클래스를 모두 정확하게 예측하는 것이 중요한 문제(예: 공공임대주택 수요자 식별)에 효과적인 해결책이 될 수 있습니다.
